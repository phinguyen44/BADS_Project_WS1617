...
source("Scripts/Helpful-Models-Threshold.R")
...
################################################################################
# Create example-dependent Cost-Matrix 


# Dimension: n * K, n being number of observations and K being number of classes
# n = nrow(df.train)
# K = 2
# cost.matrix = matrix(NA, nrow = n, ncol = K) # init



# Using Fixed Cost Matrix!

# Formula: Rows are Predictions, cols are true
costs = data.frame(matrix(c(0, 2, 1, 0), nrow = 2))
rownames(costs) = colnames(costs) = c("NoReturn", "Return")
