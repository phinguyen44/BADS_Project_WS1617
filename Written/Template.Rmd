...
A major obstacle for companies participating in the rapidly rising E-Commerce sector are excessive product returns. Up to ... % of the total sales are affected by this phenomenon (http://www.prism.gatech.edu/~bt71/articles/forecasting.pdf). 

Historically, rising product returns have been often regarded as a necessary evil (quote same named paper) that is a byproduct of equivalently rising sales. Nowadays, having customer- and product specific data en masse in combination with easily available computational power and sophisticated machine learning algorithms, 
...
Therefore it would be desirable to create a tradeoff between the mentioned advantages on the customer side and the losses on the corporate side. In order to do so, the drivers of product returns must be laid out in advance. These are often reported to be uncertainty over a product, recurrently expressed as a missfit, fraudulent behavior in terms of using the delivered item for a certain occasion and returning it afterwards as well as damage inflicted by the customer or defects (Potdar). 

Several measures intended to encounter such behavior have been proposed by Urbanke, Kranz and Kolbe. Suggested counter-measures include increased delivery time, moral suasion or transaction denial. Proposing limited payment options is in line with Pur et al., who find that Vorkasse, Sofortueberweisung and Lastschrift present a higher restriction to product returns and therefore decrease the likelihood of such (todo: accurately translate these terms).
...
Such a forecast may supply companies with information on the reverse product flow and increase their inventory control (https://www.sciencedirect.com/science/article/pii/S0377221707011575).
...
Lessmann et al. enrich the literature with applied ensemble models in churn and customer return prediction tasks. They show that the overall performance is generally superior to each of the constituent's contribution. The accuracy increases with higher diversity among the underlying models (slack paper). Additional insight in the construction of data-driven prediction systems are provided, as linear data growth may cause exponentially increased computation time.

Urbanke, Kolbe and Kranz present a new framework to large-scale feature extraction. They mitigate common problems caused by sparse data matrices using dimensionality reduction in terms of Mahalanobis Feature Extraction. They report an outperformance compared to standard machine learning techniques.
...
The latter is picked up by Potdar, who classifies product returns in the Consumer Electronics Industry according to the reason for which they have been returned. Especially Data Envelopment Analysis and Linear Regression are used for reason-specific analysis. 
