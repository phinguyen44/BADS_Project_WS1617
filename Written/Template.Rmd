...
Therefore it would be desirable to create a tradeoff between the mentioned advantages on the customer side and the losses on the corporate side. In order to do so, the drivers of product returns must be laid out in advance. These are often reported to be uncertainty over a product, recurrently expressed as a missfit, fraudulent behavior in terms of using the delivered item for a certain occasion and returning it afterwards as well as damage inflicted by the customer. The last two points are regarded as an abuse of customer protection rights (Urbanke ...). 

Several measures intended to encounter such behavior have been proposed by Urbanke, Kranz and Kolbe. They suggest the restriction of payment options, which is in line with Pur et al., who find that Vorkasse, Sofortueberweisung and Lastschrift present a higher restriction to product returns and therefore decrease the likelihood of such.

In order to encounter the problem Urbanke, Kranz and Kolbe suggest the following points:
...
Urbanke, Kolbe and Kranz present a new framework to large-scale feature extraction. They mitigate common problems caused by sparse matrices by dimensionality reduction using 'Mahalanobis Feature Extraction'. They report an outperformance compared to standard machine learning techniques.

Badea uses ANN to predict consumer behavior in Romania. Using a publically available dataset on household as well as individual level, it is investigated whether the individuals would save data through bank deposits. The resulting predictions of the ANN are found to substantially increase accuracy compared to the ones of classical discriminant analysis. 

Lessmann et al. enrich the literature with applied ensemble models in churn and customer return prediction tasks. They show that the performance is generally superior to the constituents. The accuracy increases with higher diversity among the underlying models (slack paper). Additionally insight in the construction of data-driven prediction systems are provided.


\subsection{Analyzing Product Returns}

http://www.prism.gatech.edu/~bt71/articles/forecasting.pdf
