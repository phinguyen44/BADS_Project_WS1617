...
Therefore it would be desirable to create a tradeoff between the mentioned advantages on the customer side and the losses on the corporate side. In order to do so, the drivers of product returns must be laid out in advance. 
Examples of such causes are uncertainty over a product, including, but not limited to incorrect fit, fraudulent behavior in terms of using the delivered item for a certain occasion and returning it afterwards as well as damage inflicted by the customer or item defects [@potdar_2009]. 

Several measures intended to counter such behavior have been proposed by @urbanke_2015. Suggested counter-measures include increased delivery time, moral suasion or transaction denial. Proposing limited payment options is in line with @pur_2013, who find that Vorkasse, Sofortueberweisung and Lastschrift present a higher restriction to product returns and therefore decrease the likelihood of such (todo: accurately translate these terms).
...
@lessmann_2016 propose the development of a machine learning system that tracks customer's shopping carts. In case of a high likelihood of product returns, a warning may be displayed. They furthermore discuss to charge a price premium that compensates the risk. 

In this paper, machine learning will be used to analyze the drivers of product returns and predict them accurately. 
...
@pur_2013 report a lack of consciousness of many companies towards the risk and profit options caused by product returns.
...
@toktay_2004 conclude that, despite the economic impact, literature on predicting product returns is generally sparse. Existing models are criticized to disregard relevant influential variables as product price, rebates, environmental conciousness and return price. 
...
In line with @toktay_2004, @urbanke_2015 report a lack of academic literature on the regarded issue. They recommend future research to rely on the usage of machine learning to predict product
returns in advance of the actual transaction. 
...
nn.iteration = 1:4
nn.size = c(4, 5, 8, 6)
nn.decay = c(1e-03, 1e-05, 1e-02, 1e-03)

rf.iteration = 1:4
rf.ntree = c(300, 350, 250, 400)
rf.mtry = rep(3, 4)
rf.nodesize = c(15, 30, 25, 15)

nn.model = data.frame("iteration" = nn.iteration, "size" = nn.size, "decay" = nn.decay)
rf.model = data.frame("iteration" = rf.iteration, "size" = rf.ntree, "decay" = rf.mtry, rf.nodesize)

t3 = knitr::kable(nn.model, format = "latex", booktabs = TRUE)
t4 = knitr::kable(rf.model, format = "latex", booktabs = TRUE)

cat(c("\\begin{table}[!htb]
    \\begin{minipage}{.5\\linewidth}
      \\caption{Neural Network Hyperparameter Tuning}
      \\bigskip
      \\centering",
        t3,
    "\\end{minipage}%
    \\begin{minipage}{.5\\linewidth}
        \\caption{Random Forest Hyperparameter Tuning}
        \\bigskip
        \\centering",
        t4,
    "\\end{minipage} 
...
```
<!-- Just keeping the old tables as a backup -->
