...
@toktay_2004 conclude that, despite the economic impact, literature on predicting product returns is generally sparse. Existing models are criticized to disregard relevant influential variables as product price, rebates, environmental consciousness and return price.
...
We note that nested resampling is a computationally intensive process. For this reason, we limit the inner loop cross-validation process to 4 iterations Additionally, we employ relatively small search spaces for hyperparameters (usually not exceeding 30 possible combinations) that are informed by theoretical considerations. In order to accelerate the overall model fitting process, we employ a parallelization wrapper around the tuning process.
